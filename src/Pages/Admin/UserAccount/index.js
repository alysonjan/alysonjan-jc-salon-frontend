...
const UserAccounts = () => {
  ...
  const onSubmitHandler = async (e) => {
    e.preventDefault()
    try {
        await axiosInstance.post('/api/user/add',formData).then(res => {
    ...
    <MainPageContainer>
      ...
      <FormContainer>
        <Form onSubmit={onSubmitHandler}>
          ...
          <StyledInput  type='Password' marginRight='10px' placeholder='password' name='password' value={password} onChange={e => onChange(e)} required/>
          <Select name='role' value={role} onChange={e => onChange(e)} required >
            <Option value=''>choose role</Option>
            <Option value='admin'>admin</Option>
            <Option value='employee'>employee</Option>
          </Select>
          <Button marginLeft='10px' type="submit" value="submit" fontSize="1rem" height="2.15rem">Add</Button>
        </Form>
      </FormContainer>
      <TableContainer>
        <UserAccountTable/>
